...
class ImagePredictor:
    ...
    def predict(self):
        ...
        elapsed_time = 0  # 初始化耗时变量，确保在任何情况下都有值
        ...
        try:
            ...
            print(f"预测过程中发生模型兼容性错误: {ae}")
            # 如果是模型兼容性问题，提供更明确的错误信息
            if "AAttn" in str(ae) and "qkv" in str(ae):
                print("YOLOv12模型与当前ultralytics库版本不兼容，请尝试更新库或使用较旧的模型版本")
            end_time = time.time()

...
class ChatAPI:
    ...
    def lan_deepseek_request(self, messages):
        ...
        try:
            ...
            if response.status_code != 200:
                ...
                return "Deepseek-R1离线不可用，请检查局域网LM-studio服务是否正常运行"
            ...
            elif 'error' in result and 'model not found' in result['error'].lower():
                print(f"Deepseek-R1模型未找到: {result}")
                return "Deepseek-R1模型未找到，请先在LM-studio中加载deepseek-r1-distill-qwen-7b模型"
            else:
                print(f"Deepseek-R1响应格式错误: {result}")
                # 如果消息内容为空，返回特定提示
                if 'choices' in result and len(result['choices']) > 0 and result['choices'][0]['message'].get('content', '') == '':
                    return "模型正在加载中，请稍后重试"
                return "Deepseek-R1响应格式错误，请检查模型输出"
        except requests.Timeout:
            print("局域网Deepseek-R1请求超时")
            return "Deepseek-R1请求超时，请检查局域网LM-studio服务是否正常运行"
    ...
    def local_deepseek_request(self, messages):
        ...
        try:
            ...
            if response.status_code != 200:
                ...
                return "Deepseek-R1离线不可用，请检查本地LM-studio服务是否正常运行"
            ...
            return "Deepseek-R1请求超时，请检查本地LM-studio服务是否正常运行"
        except requests.ConnectionError:
    ...
    def local_qwen25_vl_request(self, messages):
        """使用本地部署的qwen2.5-vl模型"""
        try:
            ...
            if 'choices' in result and len(result['choices']) > 0:
                ...
                print(f"qwen2.5-vl模型未找到: {result}")
    ...
    def lan_qwen25_vl_request(self, messages):
    ...
    def local_qwen25_omni_request(self, messages):
        """使用本地部署的qwen2.5-omni-3b模型"""
        try:
            print(f"正在向本地qwen2.5-omni-3b模型发送请求...")
            ...
            if response.status_code != 200:
                print(f"qwen2.5-omni-3b请求失败: 状态码 {response.status_code}, 响应: {response.text}")
                return "Qwen2.5-Omni离线不可用，请检查本地LM-studio服务是否正常运行"
                
            result = response.json()
            print(f"收到qwen2.5-omni-3b模型响应: {str(result)[:100]}...")
            
            if 'choices' in result and len(result['choices']) > 0:
                return result['choices'][0]['message']['content']
            elif 'error' in result and 'model not found' in result['error'].lower():
                print(f"qwen2.5-omni-3b模型未找到: {result}")
                return "Qwen2.5-Omni模型未找到，请先在LM-studio中加载qwen2.5-omni-3b模型"
            else:
                print(f"qwen2.5-omni-3b响应格式错误: {result}")
                return "Qwen2.5-Omni响应格式错误，请检查模型输出"
        except requests.Timeout:
            print("qwen2.5-omni-3b请求超时")
            return "Qwen2.5-Omni请求超时，请检查本地LM-studio服务是否正常运行"
        except requests.ConnectionError:
            print("无法连接到LM-studio服务")
            return "Qwen2.5-Omni离线不可用，请检查本地LM-studio服务是否正常运行"
        except Exception as e:
            print(f"本地qwen2.5-omni-3b请求失败: {e}")
            return "Qwen2.5-Omni离线不可用，请检查本地LM-studio服务是否正常运行"

    def lan_qwen25_omni_request(self, messages):
        """使用局域网部署的qwen2.5-omni-3b模型"""
        try:
            print(f"正在向局域网qwen2.5-omni-3b模型发送请求...")
            ...
            if response.status_code != 200:
                print(f"局域网qwen2.5-omni-3b请求失败: 状态码 {response.status_code}, 响应: {response.text}")
                return "Qwen2.5-Omni离线不可用，请检查局域网LM-studio服务是否正常运行"
                
            result = response.json()
            print(f"收到局域网qwen2.5-omni-3b模型响应: {str(result)[:100]}...")
            ...
            print("局域网qwen2.5-omni-3b请求超时")
            return "Qwen2.5-Omni请求超时，请检查局域网LM-studio服务是否正常运行"
        except requests.ConnectionError:
            print("无法连接到局域网qwen2.5-omni-3b服务")
            return "Qwen2.5-Omni离线不可用，请检查局域网LM-studio服务是否正常运行"
        except Exception as e:
            print(f"局域网qwen2.5-omni-3b请求失败: {e}")
            return "Qwen2.5-Omni离线不可用，请检查局域网LM-studio服务是否正常运行"
...
if __name__ == "__main__":
    ...
    # 调用本地qwen2.5-vl
    print("\n本地qwen2.5-vl响应:")
    print(chat.local_qwen25_vl_request(messages))
    
    # 调用局域网qwen2.5-vl
    print("\n局域网qwen2.5-vl响应:")
    print(chat.lan_qwen25_vl_request(messages))

    # 调用本地qwen2.5-omni-3b
    print("\n本地qwen2.5-omni-3b响应:")
    print(chat.local_qwen25_omni_request(messages))
    
    # 调用局域网qwen2.5-omni-3b
    print("\n局域网qwen2.5-omni-3b响应:")
    print(chat.lan_qwen25_omni_request(messages))
